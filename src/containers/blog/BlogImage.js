...
class BlogImage extends Component {
  ...
  render() {
    ...
    return image ? (
      ...
      {this.state.loaded ? (
        <div className="img" style={{backgroundImage: `url(${image.file.url})`}}></div>
      ) : (
        <div className="img-loading"></div>
      )}
      <img style={this.state.loaded ? {display: 'none'} : {}}
           src={image.file.url}
           alt={`Strataspatial - ${image.title}`}
           onLoad={() => setTimeout(function() { this.setState({loaded: true})}.bind(this), 500)} />
      </div>
    ) : null
  }
}
